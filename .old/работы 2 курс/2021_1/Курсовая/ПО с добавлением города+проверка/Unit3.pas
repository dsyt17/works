...
procedure TForm3.Button1Click(Sender: TObject);
  begin
      adoquery1.Close;
      adoquery1.sql.Clear;
      adoquery1.sql.add('SELECT distinct Город.Название_города, Страна.Название, Город.Описание');
      adoquery1.sql.add('FROM (((Часть_света INNER JOIN ((Климат INNER JOIN Страна ON Климат.[idClimate] = Страна.[Климат]) INNER JOIN (Направленность_отдыха INNER JOIN Страна_направленность'+' ON Направленность_отдыха.[idN] = Страна_направленность.[idN]) ON Страна.[idCountry] = Страна_направленность.[idCountry]) ON ');
      adoquery1.sql.add('Часть_света.[idPlace] = Страна.[Часть_света]) INNER JOIN (Сезоны INNER JOIN Рек_сезон ON Сезоны.[idSeason] = Рек_сезон.[idSeason]) '+'ON Страна.[idCountry] = Рек_сезон.[idCountry]) INNER JOIN (Размеры_городов INNER JOIN Город ON Размеры_городов.[idSize] = Город.[Размер города]) ON Страна.[idCountry] =');
      adoquery1.sql.add('Город.[Страна]) INNER JOIN (Инфраструктура INNER JOIN Инфраструктура_город ON Инфраструктура.[idR] = Инфраструктура_город.[idR]) ON Город.[idCity] = Инфраструктура_город.[idCity]');
      adoquery1.sql.add('where (Часть_света.Часть_света=:Place1 or Часть_света.Часть_света=:Place2 or Часть_света.Часть_света=:Place3 or Часть_света.Часть_света=:Place4 or Часть_света.Часть_света=:Place5)');
      adoquery1.sql.add('and (Сезоны.Сезон=:Season)');
      adoquery1.sql.add('and (Тип_климата=:Climate1 or Тип_климата=:Climate2 or Тип_климата=:Climate3 or Тип_климата=:Climate4 or Тип_климата=:Climate5 or Тип_климата=:Climate6 or Тип_климата=:Climate7 or Тип_климата=:Climate8 or Тип_климата=:Climate9)');
      adoquery1.sql.add('and (Направленность_отдыха.Название_направленности=:Napra1 or Направленность_отдыха.Название_направленности=:Napra2 or Направленность_отдыха.Название_направленности=:Napra3 or Направленность_отдыха.Название_направленности=:Napra4)');
      adoquery1.sql.add('and (Страна.Отличие_культуры=:Culture1 or Страна.Отличие_культуры=:Culture2)');
      adoquery1.sql.add('and (Размеры_городов.Размер=:Size1 or Размеры_городов.Размер=:Size2 or Размеры_городов.Размер=:Size3 or Размеры_городов.Размер=:Size4)');
      adoquery1.sql.add('and (Название_инф=:Inf1 or Название_инф=:Inf2 or Название_инф=:Inf3 or Название_инф=:Inf4 or Название_инф=:Inf5 or Название_инф=:Inf6 or Название_инф=:Inf7 or Название_инф=:Inf8 or Название_инф=:Inf9)');
      {adoquery1.sql.add('group by Название_города'); }
      ...
      DBGrid1.Columns[1].Title.Caption:='Страна';
      ...
      if (DBGrid1.DataSource.DataSet.IsEmpty) and (combobox1.text='Выберите подходящий сезон') then begin
      showmessage('Выберите сезон!');
      end;

      if (DBGrid1.DataSource.DataSet.IsEmpty) and NOT(combobox1.text='Выберите подходящий сезон') then begin
...
begin
showmessage('Климат является универсальным лечебным фактором на курорте.'+' Недаром по правилам курортографии климатогеографическая характеристика курорта описывается в первую очередь. Все климатические факторы (атмосферное давление, температура, влажность, ветер, солнечная радиация и др.)'+
             'определяемые высотой над уровнем моря, широтой и долготой местности,'+' наряду с особенностями ландшафта составляют среду, действующую на человека в совокупности, а ландшафтно-климатические характеристики создают тот фон, на котором наиболее эффективно действуют и другие лечебные курортные факторы.');
end;

procedure TForm3.Button5Click(Sender: TObject);
begin
showmessage('Отличие от европейской культуры. Например, если, приехав в какую-либо страну ближнего востока, Вы не хотите быть неприятно удивлены их законами и традициями, выберите "Нет" в данном списке.');
end;

procedure TForm3.Button6Click(Sender: TObject);
begin
showmessage('Размер	               Описание'+#13#10+
...
begin
showmessage('Добро пожаловать на форму выбора города!'+#13#10+'Отметьте подходящие вам параметры галочкой и нажмите кнопку "Показать города".'+
            ' Если один из наборов параметров не важен для вас - не отмечайте в нем ничего.');
end;
